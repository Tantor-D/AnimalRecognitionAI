...
class FacenetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # image.show()
        image = self.get_random_data(image, [self.image_height, self.image_width])
        # todo 好像做了变色的数据增强，考虑删除这一部分
        # cv2.imwrite("xxxx.jpg", image)
        image = np.transpose(np.asarray(image).astype(np.float64), [2, 0, 1]) / 255
        # todo 底下这段imwrite()代码会报错
        # cv2.imwrite("yyyy.jpg", image)
        if self.channel == 1:
            images[1, 0, :, :] = image
        else:
            images[1, :, :, :] = image
    ...
    def get_lfw_paths(self, lfw_dir, file_ext="jpg"):
        ...
        for i in range(len(pairs)):
            ...
            if os.path.exists(path0) and os.path.exists(path1):  # Only add the pair if both paths exist
                ...
                print(f"don't exist,path0:{path0}, path1:{path1}")
